...
export default function DashboardSidebar({ isOpen }: DashboardSidebarProps) {
  ...
  return (
    <>
      ...
      {/* Optional: Floating toggle button when sidebar is closed - Uncomment if needed */}
      {/* {!isOpen && (
        <button
          onClick={() => setIsOpen(true)}
          className="fixed left-2 top-24 z-50 w-8 h-8 bg-white border border-[#EAECF0] rounded-lg shadow-md flex items-center justify-center hover:bg-gray-50 transition-all duration-200"
        >
          <Menu size={16} className="text-[#667085]" />
        </button>
      )} */}
    </>
  );
